...
import { handleOrderPaid } from '../events/handlers/orderPaid.js';
...
// @access  Public
export const consumeOrderPaidMessage = async (req, res) => {
  try {
    ...
    if (queueInfo.messageCount === 0) {
      return res.json({
        success: true,
        message: 'Aucun message order.paid dans la queue',
        data: { 
    ...
    try {
      ...
      res.json({
        success: true,
        message: 'Message order.paid traité avec succès',
        data: {
          messageProcessed: true,
          payload: data,
          actions: [
            'Email du payement  de command envoyer',
